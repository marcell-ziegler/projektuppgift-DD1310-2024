...
class Carriage:
    ...
    def __init__(self, seating_configuration: str, num_rows: int, carriage_num: int):
        ...
        self.left_seats, self.right_seats = (
        ...
        for _ in range(num_rows):
            left = []
            for _ in range(self.left_seats):
                seat_num += 1
                left.append(Seat(seat_num))

            right = []
            for _ in range(self.right_seats):
                seat_num += 1
    ...
    def get_seat_num(self, seat_num: int) -> Seat:
        ...
        # Total row width
        row_width = self.left_seats + self.right_seats
        ...
        return self.seats[row - 1][1][row_index - 1 - self.left_seats]
    ...
    def get_seat_name(self, passenger_name: str) -> Seat:
        ...
        seat_filter = filter(
            ...
            itertools.chain(*itertools.chain(*self.seats)),  # Flatten the list of seats
        )

        matches = list(seat_filter)

        if len(matches) < 1:
            raise ValueError(f"No seat found for passenger {passenger_name}")
